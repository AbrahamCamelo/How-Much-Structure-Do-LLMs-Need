...
from .constants import (
    ...
    DEFAULT_MIN_CLUSTER_PROPORTION,
...
from .descriptions import (
    build_description_dataset_name,
...
def _pipeline_output_path(
    ...
    return (
        ...
        / Path(scopus_file).name
    )


def _workflow_summary_path(
    kind: str,
    provider: str,
    scopus_file: str | Path,
    significant_papers_per_cluster: int = DEFAULT_SIGNIFICANT_PAPERS_PER_CLUSTER,
...
def _expected_description_artifacts(
    ...
    for pipeline_name in PIPELINE_NAMES:
        description_path = _pipeline_output_path(
            pipeline_name,
            provider,
            file_name,
            significant_papers_per_cluster=significant_papers_per_cluster,
        ...
        artifacts.append(
            DescriptionArtifact(
                ...
                dataset_name=build_description_dataset_name(description_path),
            )
        )
...
def _observed_cluster_count(labeled_scopus_df: pd.DataFrame) -> int:
    return int(pd.to_numeric(labeled_scopus_df["cluster"].dropna(), errors="raise").astype(int).nunique())
...
def _resolve_clustered_scopus_for_descriptions(
    ...
    if only_missing and clustered_scopus_file.exists():
        try:
            ...
            clustering_method_values = (
                labeled_scopus_df["clustering_method"].dropna().astype(str).str.strip().str.lower().unique().tolist()
                if "clustering_method" in labeled_scopus_df.columns
                else []
            )
            clustering_resolution_values = (
                pd.to_numeric(labeled_scopus_df["clustering_resolution"].dropna(), errors="coerce").unique().tolist()
                if "clustering_resolution" in labeled_scopus_df.columns
                else []
            )
            clustering_requested_resolution_values = (
                pd.to_numeric(
                    labeled_scopus_df["clustering_requested_resolution"].dropna(),
                    errors="coerce",
                ).unique().tolist()
                if "clustering_requested_resolution" in labeled_scopus_df.columns
                else clustering_resolution_values
            )
            clustering_max_cluster_values = (
                pd.to_numeric(
                    labeled_scopus_df["clustering_max_clusters"].dropna(),
                    errors="coerce",
                ).unique().tolist()
                if "clustering_max_clusters" in labeled_scopus_df.columns
                else []
            )
            clustering_min_cluster_size_values = (
                pd.to_numeric(
                    labeled_scopus_df["clustering_min_cluster_size"].dropna(),
                    errors="coerce",
                ).unique().tolist()
                if "clustering_min_cluster_size" in labeled_scopus_df.columns
                else []
            )
            clustering_min_cluster_basis_values = (
                labeled_scopus_df["clustering_min_cluster_proportion_basis"].dropna().astype(str).str.strip().str.lower().unique().tolist()
                if "clustering_min_cluster_proportion_basis" in labeled_scopus_df.columns
                else []
            )
            clustering_selection_strategy_values = (
                labeled_scopus_df["clustering_cluster_selection_strategy"].dropna().astype(str).str.strip().str.lower().unique().tolist()
                if "clustering_cluster_selection_strategy" in labeled_scopus_df.columns
                else []
            )
            clustering_min_cluster_proportion_values = (
                pd.to_numeric(
                    labeled_scopus_df["clustering_min_cluster_proportion"].dropna(),
                    errors="coerce",
                ).unique().tolist()
                if "clustering_min_cluster_proportion" in labeled_scopus_df.columns
                else []
            )
            uses_direct_citation = clustering_method_values == ["direct_citation"]
            uses_expected_requested_resolution = (
                len(clustering_requested_resolution_values) == 1
                and abs(float(clustering_requested_resolution_values[0]) - float(resolution)) <= 1e-9
            )
            uses_expected_min_cluster_size = (
                len(clustering_min_cluster_size_values) == 1
                and int(float(clustering_min_cluster_size_values[0])) == int(min_cluster_size)
            )
            uses_expected_min_cluster_basis = (
                len(clustering_min_cluster_basis_values) == 0
                if min_cluster_proportion is None
                else clustering_min_cluster_basis_values == ["largest_cluster"]
            )
            uses_expected_selection_strategy = (
                len(clustering_selection_strategy_values) == 0
                if max_cluster_count is None
                else (
                    clustering_selection_strategy_values == ["largest_clusters"]
                    or (
                        len(clustering_selection_strategy_values) == 0
                        and int(pd.to_numeric(labeled_scopus_df["cluster"].dropna(), errors="coerce").nunique()) <= int(max_cluster_count)
                    )
                )
            )
            uses_expected_min_cluster_proportion = (
                len(clustering_min_cluster_proportion_values) == 0
                if min_cluster_proportion is None
                else (
                    len(clustering_min_cluster_proportion_values) == 1
                    and abs(
                        float(clustering_min_cluster_proportion_values[0])
                        - float(min_cluster_proportion)
                    ) <= 1e-9
                )
            )
            uses_expected_max_cluster_cap = (
                len(clustering_max_cluster_values) == 0
                if max_cluster_count is None
                else (
                    len(clustering_max_cluster_values) == 1
                    and int(float(clustering_max_cluster_values[0])) == int(max_cluster_count)
                )
            )
            if (
                uses_direct_citation
                and uses_expected_requested_resolution
                and uses_expected_min_cluster_size
                and uses_expected_min_cluster_basis
                and uses_expected_selection_strategy
                and uses_expected_min_cluster_proportion
                and uses_expected_max_cluster_cap
            ):
...
def _build_description_summary(
    ...
    provider: str,
    significant_papers_per_cluster: int,
    query: str,
    raw_scopus_file: Path,
    clustered_scopus_file: Path,
    target_clusters: int,
    ...
    observed_cluster_count = _observed_cluster_count(labeled_scopus_df)
    ...
    effective_min_cluster_size_values = (
        pd.to_numeric(
            labeled_scopus_df["clustering_effective_min_cluster_size"].dropna(),
            errors="coerce",
        ).unique().tolist()
        if "clustering_effective_min_cluster_size" in labeled_scopus_df.columns
        else []
    )
    min_cluster_basis_values = (
        labeled_scopus_df["clustering_min_cluster_proportion_basis"].dropna().astype(str).str.strip().unique().tolist()
        if "clustering_min_cluster_proportion_basis" in labeled_scopus_df.columns
        else []
    )
    selection_strategy_values = (
        labeled_scopus_df["clustering_cluster_selection_strategy"].dropna().astype(str).str.strip().unique().tolist()
        if "clustering_cluster_selection_strategy" in labeled_scopus_df.columns
        else []
    )
    min_cluster_proportion_values = (
        pd.to_numeric(
            labeled_scopus_df["clustering_min_cluster_proportion"].dropna(),
            errors="coerce",
        ).unique().tolist()
    ...
    requested_resolution_values = (
        pd.to_numeric(labeled_scopus_df["clustering_requested_resolution"].dropna(), errors="coerce").unique().tolist()
        if "clustering_requested_resolution" in labeled_scopus_df.columns
        else []
    )
    max_cluster_values = (
        pd.to_numeric(labeled_scopus_df["clustering_max_clusters"].dropna(), errors="coerce").unique().tolist()
        if "clustering_max_clusters" in labeled_scopus_df.columns
        else []
    )
    ...
    louvain_requested_resolution: object = (
        float(requested_resolution_values[0])
        if len(requested_resolution_values) == 1
        else pd.NA
    )
    louvain_max_clusters: object = (
        int(float(max_cluster_values[0]))
        if len(max_cluster_values) == 1
        else pd.NA
    )
    louvain_min_cluster_size: object = (
        int(float(min_cluster_size_values[0]))
        if len(min_cluster_size_values) == 1
        else pd.NA
    )
    louvain_effective_min_cluster_size: object = (
        int(float(effective_min_cluster_size_values[0]))
        if len(effective_min_cluster_size_values) == 1
        else pd.NA
    )
    louvain_min_cluster_basis: object = (
        str(min_cluster_basis_values[0])
        if len(min_cluster_basis_values) == 1
        else pd.NA
    )
    louvain_cluster_selection_strategy: object = (
        str(selection_strategy_values[0])
        if len(selection_strategy_values) == 1
        else pd.NA
    )
    louvain_min_cluster_proportion: object = (
        float(min_cluster_proportion_values[0])
        if len(min_cluster_proportion_values) == 1
        else pd.NA
    )

    summary_rows: list[dict[str, object]] = []
    experiment_variant = experiment_variant_name(significant_papers_per_cluster)
        ...
        summary_rows.append(
            {
                ...
                "louvain_min_cluster_size": louvain_min_cluster_size,
                "louvain_effective_min_cluster_size": louvain_effective_min_cluster_size,
                "louvain_min_cluster_proportion": louvain_min_cluster_proportion,
                "louvain_min_cluster_proportion_basis": louvain_min_cluster_basis,
                "louvain_cluster_selection_strategy": louvain_cluster_selection_strategy,
                "louvain_requested_resolution": louvain_requested_resolution,
                "louvain_resolution": louvain_resolution,
                "louvain_max_clusters": louvain_max_clusters,
...
def run_all_descriptions(
    ...
    human_description_file = None
    target_clusters = _observed_cluster_count(labeled_scopus_df)
...
def run_all_evaluations(
    ...
    if clustering_method_values != ["direct_citation"]:
        raise ValueError(
            "The clustered Scopus file was not generated with the direct citation workflow. "
            f"Please rerun description generation for {resolved_scopus_file.name} before running evaluations."
        )
    ...
    for artifact in description_artifacts:
        ...
        coverage_error = ""
        induced_output_path = (
            evaluation_variant_dir(QUALITY_DIR, significant_papers_per_cluster)
            / "results"
            / f"{artifact.dataset_name}_induced_clusters.csv"
        )
        coverage_output_path = (
            evaluation_variant_dir(COVERAGE_DIR, significant_papers_per_cluster)
            / "results"
            / f"{artifact.dataset_name}_coverage.csv"
        )
        induced_silhouette_path = (
            evaluation_variant_dir(SILHOUETTE_DIR, significant_papers_per_cluster)
            / f"{artifact.dataset_name}_induced_clusters_silhouette.csv"
        )
        induced_modularity_path = (
            evaluation_variant_dir(MODULARITY_DIR, significant_papers_per_cluster)
            / f"{artifact.dataset_name}_induced_clusters_modularity.csv"
        )
        similarity_output_name = f"{artifact.dataset_name}__vs__{clustered_scopus_file.stem}"
        similarity_output_path = (
            evaluation_variant_dir(SIMILARITY_DIR, significant_papers_per_cluster)
            / f"{similarity_output_name}.csv"
        )
        ...
        summary_rows.append(
            {
                ...
                "louvain_silhouette_file": _display_name(
                    evaluation_variant_dir(SILHOUETTE_DIR, significant_papers_per_cluster)
                    / f"{clustered_scopus_file.stem}_silhouette.csv"
                ),
                ...
                "louvain_modularity_file": _display_name(
                    evaluation_variant_dir(MODULARITY_DIR, significant_papers_per_cluster)
                    / f"{clustered_scopus_file.stem}_modularity.csv"
                ),
            }
        )

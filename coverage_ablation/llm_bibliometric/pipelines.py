...
def _save_pipeline_output(
    ...
    pipeline_dir = ensure_directory(DESCRIPTION_DIR / pipeline_name / provider.lower())
...
def run_pipeline_1(
    ...
    return _save_pipeline_output(output_df, "pipeline_1", provider, output_file)
...
def run_pipeline_2(
    ...
    return _save_pipeline_output(output_df, "pipeline_2", provider, output_file)
...
def run_pipeline_3(
    ...
    return _save_pipeline_output(output_df, "pipeline_3", provider, output_file)
...
def run_pipeline_4(
    ...
    return _save_pipeline_output(output_df, "pipeline_4", provider, output_file)
...
def run_pipeline_5(
    ...
    paper_cluster_lookup = dict(
        zip(
            labeled_only_df["paper_id"].astype(int).tolist(),
            pd.to_numeric(labeled_only_df["cluster"], errors="raise").astype(int).tolist(),
        )
    )
    for _, row in references_df.iterrows():
        cluster_id = int(row["cluster_id"])
        ...
        invalid_reference_ids = [
            ...
            if paper_cluster_lookup.get(reference_id) != cluster_id
        ]
        if invalid_reference_ids:
            invalid_tokens = [f"[{reference_id}]" for reference_id in invalid_reference_ids]
            raise ValueError(
                f"pipeline_5 step 1 selected references outside cluster {cluster_id}: {invalid_tokens}"
            )
    ...
    output_df = output_df[["cluster_id", "description", "references"]]
    return _save_pipeline_output(output_df, "pipeline_5", provider, output_file)
...
def run_pipeline_6(
    ...
    return _save_pipeline_output(output_df, "pipeline_6", provider, output_file)

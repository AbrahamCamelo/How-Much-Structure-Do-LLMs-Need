...
def _workflow_summary_path(
    ...
    target_dir = ensure_directory(
        full_run_output_dir(kind, provider, prompts_file, embedding_model)
    )
        ...
        summary_rows.append(
            {
                ...
                "provider": provider,
                "prompts_variant": prompts_variant or "default",
                "query": query,
...
def run_all_descriptions(
    ...
    prompts_variant = prompt_variant_name(prompts_file)
    ...
    summary_df = _build_description_summary(
        outputs=outputs,
        provider=canonical_provider,
        prompts_variant=prompts_variant,
        query=query,
...
def run_all_descriptions_for_all_scopus(
    ...
    summary_path = ensure_directory(
        full_run_output_dir("descriptions", canonical_provider, prompts_file)
    ) / (
...
def run_all_evaluations(
    ...
    for artifact in description_artifacts:
        ...
        coverage_error = ""
        induced_output_path = (
            evaluation_artifact_dir(QUALITY_DIR, embedding_model)
            / "results"
            / f"{artifact.dataset_name}_induced_clusters.csv"
        )
        coverage_output_path = (
            evaluation_artifact_dir(COVERAGE_DIR, embedding_model)
            / "results"
            / f"{artifact.dataset_name}_coverage.csv"
        )
        induced_silhouette_path = (
            evaluation_artifact_dir(SILHOUETTE_DIR, embedding_model)
            / f"{artifact.dataset_name}_induced_clusters_silhouette.csv"
        )
        induced_modularity_path = (
            evaluation_artifact_dir(MODULARITY_DIR, embedding_model)
        ...
        similarity_output_path = (
            evaluation_artifact_dir(SIMILARITY_DIR, embedding_model)
        ...
        summary_rows.append(
            {
                ...
                "provider": canonical_provider,
                "prompts_variant": prompts_variant or "default",
                "embedding_variant": embedding_variant or "default",
                "embedding_model": embedding_model,

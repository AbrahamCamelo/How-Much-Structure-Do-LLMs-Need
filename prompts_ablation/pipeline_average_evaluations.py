...
from llm_bibliometric.output_layout import (
...
def _default_input_dir(
    ...
    return full_run_output_dir("evaluations", provider, prompts_file, embedding_model)
...
def build_pipeline_average_evaluations(
    ...
    resolved_input_dir = (
        ...
        else _default_input_dir(canonical_provider, prompts_file, embedding_model)
    )
    ...
    if not include_human:
        work_df = work_df[work_df["source_name"].astype(str).str.startswith("pipeline_")].copy()
        if work_df.empty:
            raise ValueError("No pipeline rows were found after excluding human descriptions.")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--include-human",
        action="store_true",
        help="Include the human_descriptions row in the aggregated output.",
    )
    return parser
